...
`基于 Boost.Asio 的跨平台多房间聊天后端`

简历描述示例：

1. 基于 `Boost.Asio` 实现跨平台 C++17 聊天后端，采用异步 TCP 通信和事件驱动网络模型。
2. 设计长度字段 + JSON 的应用层协议，解决 TCP 粘包/拆包问题，支持登录、房间切换、群聊、私聊和心跳检测。
3. 支持 `MySQL` 离线私聊消息存储与自动补发，支持 `Redis` 在线状态维护与 TTL 刷新。
4. 使用 `BlockingQueue + ThreadPool` 构建业务任务队列，并通过单例执行器异步处理持久化与状态同步任务，避免阻塞网络线程。
5. 使用 `Strategy + Factory + Null Object + Singleton` 重构存储层，按配置切换后端实现，提升可扩展性与可测试性。
