...
namespace asiochat::server
{
    namespace
    {
        ...
        std::string join_url(const AppConfig::RoomAiAgentConfig &config)
        {
            const std::string base = trim_trailing_slash(
                config.base_url.empty() ? default_base_url(config.provider) : config.base_url);

            const std::string endpoint =
                config.endpoint.empty() ? default_base_url(config.provider) : config.endpoint;
        ...
        std::string build_prompt_with_context(
            const AppConfig::RoomAiAgentConfig &config,
            ...
            std::string_view message)
        {
            ...
            prompt << "Reply as "
                   << (config.bot_name.empty() ? "the room bot" : config.bot_name)
                   << " in a concise, helpful way.";
            return prompt.str();
        }

        std::string call_openai_compatible_api(
            const AppConfig::RoomAiAgentConfig &config,
            const std::vector<std::string> &recent_messages,
            std::string_view room,
            std::string_view from_user,
            std::string_view message)
        {
            ...
            user_message["content"] = build_prompt_with_context(config,
                                                                recent_messages, room, from_user, message);
    ...
    RoomAiAgentService::RoomState *
    RoomAiAgentService::ensure_room_state_(std::string_view room)
    {
                                           ...
                                           ReplyHandler handler)
    {
        ...
        auto task = [this,
                     ...
                     room_state_snapshot = std::move(room_state_snapshot),
                     handler = std::move(handler)]() mutable
        {
            ...
            {
                std::string fallback =
                    build_local_reply_text(room_state_snapshot, room, from_user, message);
                fallback += "\n[provider-fallback] " + std::string(e.what());
    ...
    void RoomAiAgentService::replace_all(std::string &text,
